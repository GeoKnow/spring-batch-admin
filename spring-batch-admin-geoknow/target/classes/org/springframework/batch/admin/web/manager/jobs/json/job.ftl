...
"job" : { 
    ...
    "name" : "${jobInfo.name}",
    "launchable" : <#if jobInfo.launchable??>${jobInfo.launchable?string}<#else>false</#if>,
    "incrementable" : <#if jobInfo.incrementable??>${jobInfo.incrementable?string}<#else>false</#if>,
    "jobInstances" : {
